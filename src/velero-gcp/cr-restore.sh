...
# Create Cloud Bucket 
gsutil mb gs://$bucketNameForVelero/
gsutil iam ch serviceAccount:cr-velero@cr-test-356813.iam.gserviceaccount.com:objectAdmin gs://${bucketNameForVelero}
...
function installVeleroOnCluster() {
        $vl uninstall --force
        installer=$($vl install --provider gcp --plugins velero/velero-plugin-for-gcp:latest --use-restic --default-volumes-to-restic --use-volume-snapshots=false --bucket "${bucketNameForVelero}" --velero-pod-mem-limit "${veleroPodMemLimit}" --velero-pod-mem-request "${veleroPodMemRequest}" --secret-file /home/ashokdas_test1/credentials-velero)
